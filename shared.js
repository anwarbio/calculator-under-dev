...
async function saveMeterReading(entry) {
  try {
    ...
    // 🔹 First, delete older entries for the same consumer+month+year
    ...
    for (const d of all) {
      if (
        d.consumerName === entry.consumerName &&
        d.billMonth === entry.billMonth &&
        d.billYear === entry.billYear
      ) {
        await new Promise((resolve, reject) => {
          const delReq = store.delete(d.entryId);
          delReq.onsuccess = () => resolve();
          delReq.onerror = reject;
        });
        console.log("🗑️ Deleted old meter entry:", d.entryId);
      }
    }
